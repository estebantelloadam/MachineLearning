...
HORIZONS = [2, 5, 10, 20, 40, 60, 250, 500, 1000]
...
def train_and_evaluate(data, predictors):
    ...
    plot_accuracy_over_time(model, train_data[predictors], train_data["Target"])
...
def plot_feature_importance(model, predictors):
    ...
    plt.bar(range(len(predictors)), importance[indices], align="center")
    plt.xticks(range(len(predictors)), np.array(predictors)[indices], rotation=45)
    plt.xlim([-1, len(predictors)])
    plt.tight_layout()
    plt.show()


def plot_accuracy_over_time(model, X, y, iterations=5):
    """Plot the accuracy of the model over time (iterations) using increasing amounts of training data."""
    test_size = 0.1
    train_size_increment = (1 - test_size) / iterations
    accuracies = []

    for i in range(1, iterations + 1):
        current_train_size = train_size_increment * i
        X_train, X_test, y_train, y_test = train_test_split(X, y, train_size=current_train_size, shuffle=True)
        model.fit(X_train, y_train)
        accuracy = model.score(X_test, y_test)
        accuracies.append(accuracy)

    plt.figure(figsize=(10, 6))
    plt.plot(range(1, iterations + 1), accuracies, marker='o', linestyle='-')
    plt.title("Accuracy over Time (Iterations)")
