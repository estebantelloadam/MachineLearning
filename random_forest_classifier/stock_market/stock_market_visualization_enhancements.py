...
if sp500 is not None:
    try:
        ...
        sp500["Close"].plot(label="Actual Close Prices", legend=True)
        predictions['Predictions'].replace({1: sp500["Close"].max(), 0: sp500["Close"].min()}).plot(
            style='.', label="Predictions (Ups and Downs)", legend=True)
        plt.title("Predicted Ups and Downs vs Actual Close Prices")
        plt.show()
